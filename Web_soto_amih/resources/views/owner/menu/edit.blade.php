...
<div class="flex flex-col items-center">
    ...
    <form action="{{ route('owner.menu.update', $menu->id) }}" method="POST" enctype="multipart/form-data" class="w-full max-w-4xl">
        @csrf
        @method('PUT')
            ...
            <div class="w-1/2 flex-shrink-0">
                <div class="bg-white rounded-2xl shadow p-5 h-full">
                    ...
                    <div id="drop-area"
                        ...
                        {{-- Preview foto lama (langsung tampil) --}}
                        <img id="preview"
                             src="{{ asset('storage/menus/' . $menu->foto) }}"
                             alt="{{ $menu->nama_produk }}"
                             class="absolute inset-0 w-full h-full object-cover rounded-2xl">

                        {{-- Overlay ganti foto --}}
                        <div id="overlay" class="absolute inset-0 bg-black bg-opacity-40 rounded-2xl flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
