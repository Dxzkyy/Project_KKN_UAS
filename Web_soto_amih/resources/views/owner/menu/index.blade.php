...
@section('content')
    ...
    <div class="flex items-center gap-2 mb-5">
        <button onclick="filterKategori('Semua')"
                class="filter-btn active px-4 py-1.5 rounded-full text-sm font-medium transition"
                data-kategori="Semua">
            Semua
        </button>
                    ...
                    <tr class="menu-row hover:bg-gray-50 transition" data-kategori="{{ $menu->kategori }}">
                        ...
                        <td class="py-4 px-4">
                            <div class="flex items-center gap-3">
                                <img src="{{ asset('storage/menus/' . $menu->foto) }}"
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400 mt-0.5">Ditambah {{ $menu->created_at->diffForHumans() }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        {{-- Stok --}}
                        <td class="py-4 px-4">
                            <span class="font-semibold {{ $menu->stok <= 5 ? 'text-red-500' : 'text-gray-700' }}">
                                {{ $menu->stok }}
                            </span>
                            @if($menu->stok <= 5)
                        ...
                        <td class="py-4 px-4">
                            <div class="flex items-center gap-2">
                                ...
                                <form id="delete-form-{{ $menu->id }}" action="{{ route('owner.menu.destroy', $menu->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button"
