...
<div class="grid grid-cols-12 gap-6 h-full">
    ...
    <div class="col-span-8 flex flex-col gap-4">
        ...
        <div class="bg-white p-4 rounded-2xl shadow-sm flex flex-col gap-4">
            <div class="relative">
                ...
                <input type="text" id="searchInput" placeholder="Cari produk (F2)" class="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-xl focus:ring-[#C97B2E] focus:border-[#C97B2E] outline-none transition">
            </div>
        ...
        <div class="grid grid-cols-3 gap-4 overflow-y-auto" style="max-height: calc(100vh - 280px);">
            ...
            <div class="bg-white rounded-2xl p-3 shadow-sm hover:shadow-md transition cursor-pointer group">
                <div class="relative h-32 mb-3 rounded-xl overflow-hidden">
                    <img src="https://via.placeholder.com/150" alt="Soto Ayam" class="w-full h-full object-cover group-hover:scale-105 transition duration-300">
                    <span class="absolute top-2 right-2 bg-blue-500 text-white text-[10px] font-bold px-2 py-1 rounded-md">Makanan</span>
                </div>
                <div class="flex justify-between items-start">
                    <div>
                        <h3 class="font-semibold text-gray-800 text-sm">Soto Ayam</h3>
                        <p class="text-xs text-gray-400 mb-1">BRG-001</p>
                        <p class="font-bold text-[#C97B2E]">Rp 25.000</p>
                        <p class="text-[10px] text-gray-500 mt-1">Stok: 120</p>
                    </div>
                    {{-- Tombol ini sudah dipasang fungsi klik tambahKeKeranjang --}}
                    <button class="bg-[#C97B2E] hover:bg-orange-600 text-white p-1.5 rounded-lg transition" onclick="tambahKeKeranjang(1, 'Soto Ayam', 25000)">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="col-span-4 bg-white rounded-2xl shadow-sm flex flex-col h-full" style="max-height: calc(100vh - 120px);">
        ...
        <div class="p-4 border-t border-gray-100 flex flex-col gap-4">
            ...
            <div class="flex items-center gap-2">
                ...
                <select id="tipeDiskon" class="border border-gray-200 rounded-lg text-sm p-1.5 focus:ring-[#C97B2E] outline-none">
                    <option value="persen">%</option>
                    <option value="nominal">Rp</option>
                </select>
                <input type="number" id="inputDiskon" placeholder="0" class="flex-1 border border-gray-200 rounded-lg text-sm p-1.5 focus:ring-[#C97B2E] outline-none text-right">
            </div>
            ...
            <div class="flex flex-col gap-2 mt-2">
                ...
                <button id="btnBayar" class="w-full bg-[#C97B2E] hover:bg-orange-600 text-white font-bold py-3 rounded-xl shadow-md transition flex justify-center items-center gap-2">
                    Bayar (F9)
                </button>
                {{-- Tombol sudah dipasang ID btnHapusKeranjang --}}
                <button id="btnHapusKeranjang" class="w-full border border-gray-200 hover:bg-red-50 text-red-500 font-semibold py-2.5 rounded-xl transition flex justify-center items-center gap-2 text-sm">
...
<script>
    ...
    document.getElementById('btnBayar').addEventListener('click', () => {
        ...
        Swal.fire({
            ...
            text: "Pastikan metode bayar dan total sudah sesuai.",
            icon: 'question',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                Swal.fire('Berhasil!', 'Pesanan sedang diproses ke dapur.', 'success').then(() => {
                    cart = [];
                    document.getElementById('inputDiskon').value = '';
                    renderCart();
                });
